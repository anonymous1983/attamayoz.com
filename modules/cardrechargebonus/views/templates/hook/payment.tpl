...
<p class="payment_module {if !$can_use_crb } cant_use {/if}">
        ...
        {if $can_use_crb }
            ...
            <a id="id_a_paybycardrechargeorbonus" href="{$link->getModuleLink('cardrechargebonus', 'payment', [], true)|escape:'html'}" title="{l s='Pay by card Recharge or Bonus' mod='cardrechargebonus'}.">
                    {l s='Pay by card Recharge or Bonus' mod='cardrechargebonus'}
            </a>
            {if (Module::isInstalled('attamayozcardmodule')) }
                <a  id="id_a_rechargeyouraccount" href="{$link->getModuleLink('attamayozcardmodule', 'account')|escape:'htmlall':'UTF-8'}" title="{l s='Recharge my account' mod='cardrechargebonus'}">
